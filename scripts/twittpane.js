...
TWITT.conf = {
  ...
  */
  ...
  extension_url: 'chrome-extension://' + TWITT.extension_id.pub +
                 '/twitter.html',
  extension_url2: 'chrome-extension://' + TWITT.extension_id.pub +
                  '/twitter.html#',
  extension_url3: 'chrome-extension://' + TWITT.extension_id.dev +
                  '/twitter.html',
  extension_url4: 'chrome-extension://' + TWITT.extension_id.dev +
                  '/twitter.html#',
  extension_pub_url: 'https://chrome.google.com/webstore/detail/' +
                      TWITT.extension_id.pub,
  tabid: null,
...
TWITT.get_words = function() {
  ...
  //console.log(TWITT.conf.saved_searches);
...
TWITT.search_tweets = function(pane_id, word) {
  ...
  if (TWITT.conf.search_data[pane_id].max_id_str === undefined) {
    ...
    var _max_id_str = new BigNumber(TWITT.conf.search_data[pane_id].max_id_str);
    var since_id_str = _max_id_str.add(1);
  }

  this.jsoauth.searchTweets(word,
                            TWITT.conf.search_timeline_get_count,
                            since_id_str,
                            function(data) {
    //console.log(data);
    var json = JSON.parse(data.text);
    ...
    if (undefined === json.status) {
      ...
      if (TWITT.conf.search_data[pane_id] === undefined) {
        ...
        TWITT.conf.search_data[pane_id].max_id_str =
          json.search_metadata.max_id_str;
      }
